...
exports.createRating = async (req, res) => {
  try {
    ...
    // Check if the user is enrolled in the product

    const productDetails = await Product.findOne({
      _id: productId,
      buyersOdered: { $elemMatch: { $eq: userId } },
    })

    if (!productDetails) {
      return res.status(404).json({
        success: false,
        message: "Buyer is not Odered this product",
      })
    }
...
exports.getAverageRating = async (req, res) => {
  try {
    ...
    const result = await RatingAndReview.aggregate([
      {
        $match: {
          product: new mongoose.Types.ObjectId(productId) // Convert productId to ObjectId
        },
      },
      {
        $group: {
